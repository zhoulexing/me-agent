...
const DEFAULT_CONFIG_PATH = join(homedir(), '.config', 'zlx-publish-wechat-article', 'config.json');
...
function usage() {
  return `用法:
  publish-wechat-article.mjs check-config [--config <配置文件>]
...
function configPath(explicitPath = '') {
  return resolve(explicitPath || process.env.WECHAT_PUBLISH_CONFIG || DEFAULT_CONFIG_PATH);
}

function readCredentialConfig(explicitPath = '') {
  const path = configPath(explicitPath);
  if (!existsSync(path)) return { path, exists: false, appId: '', appSecret: '' };
  ...
  try {
    ...
    throw new Error(`公众号凭证配置不是有效 JSON: ${path} (${error.message})`);
  }

  const appId = String(data.appId || '').trim();
  const appSecret = String(data.appSecret || '').trim();
  return { path, exists: true, appId, appSecret };
}

function credentials(explicitPath = '') {
  const envAppId = String(process.env.WECHAT_APP_ID || '').trim();
  const envAppSecret = String(process.env.WECHAT_APP_SECRET || '').trim();
  if (envAppId || envAppSecret) {
    if (!envAppId || !envAppSecret) {
      throw new Error('WECHAT_APP_ID 和 WECHAT_APP_SECRET 必须同时设置');
    }
    return { appId: envAppId, appSecret: envAppSecret, source: 'environment', path: null };
  }
  ...
  return { appId: config.appId, appSecret: config.appSecret, source: 'config-file', path: config.path };
}
...
async function imageBlob(source, baseDir) {
  ...
  if (/^https?:\/\//i.test(source)) {
    const response = await fetchChecked(source, {
      headers: { 'user-agent': 'Mozilla/5.0 WeChatArticlePublisher/1.0' },
    });
...
async function uploadAndReplaceImages(token, html, baseDir) {
  ...
  for (const match of matches) {
    ...
    if (/^https?:\/\/mmbiz\.qpic\.cn/i.test(source)) chunks.push(originalTag);
    else {
...
function integerOption(value, fallback) {
  ...
  if (!Number.isInteger(parsed) || ![0, 1].includes(parsed)) {
    throw new Error(`选项值必须为 0 或 1: ${value}`);
  }
  return parsed;
}
...
if (!['check-config', 'draft', 'update'].includes(command)) fail(usage());

try {
  if (command === 'check-config') {
    const credential = credentials(String(args.config || ''));
    process.stdout.write(`${JSON.stringify({
      success: true,
      configured: true,
      source: credential.source,
      configPath: credential.path,
    }, null, 2)}\n`);
    process.exit(0);
  }
  ...
  const { appId, appSecret } = credentials(String(args.config || ''));
