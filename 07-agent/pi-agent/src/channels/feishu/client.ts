...
export class FeishuClient {
  ...
  async sendCardReference(target: FeishuSendTarget, cardId: string): Promise<string> {
    return this.withCardReferenceRetry(() =>
      this.sendMessageContent(target, "interactive", {
        type: "card",
        data: {
          card_id: cardId
        }
  ...
  private async withCardReferenceRetry(task: () => Promise<string>): Promise<string> {
    ...
    for (let attempt = 0; attempt <= delays.length; attempt += 1) {
      try {
        ...
        lastError = error;
        if (!isCardReferenceSyncError(error) || attempt >= delays.length) break;
...
function isCardReferenceSyncError(error: unknown): boolean {
  const text = getErrorMessage(error).toLowerCase();
  const payload = safeStringify(error).toLowerCase();
  return text.includes("cardid is invalid") || text.includes("card_id") || payload.includes("11310") || payload.includes("cardid is invalid");
}
