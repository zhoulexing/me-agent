...
import { PiAgentClient, type PiStreamEvent } from "./client.js";
...
export class AgentRuntime {
  ...
  constructor(private readonly options: AgentRuntimeOptions) {
    this.client = options.client ?? new PiAgentClient();
  ...
  run(params: AgentRunParams, callbacks: AgentRunCallbacks = {}): Promise<AgentRunResult> {
    ...
    return this.queue.enqueue(queueKey, () => this._run(params, { ...callbacks, stream: false }));
  }

  runStream(params: AgentRunParams, callbacks: AgentRunCallbacks = {}): Promise<AgentRunResult> {
    const queueKey = buildQueueKey(params);
    return this.queue.enqueue(queueKey, () => this._runStream(params, callbacks));
  }

  private _runStream(params: AgentRunParams, callbacks: AgentRunCallbacks): Promise<AgentRunResult> {
    return this._run(params, { ...callbacks, stream: true });
  }
  ...
  private async _run(params: AgentRunParams, callbacks: AgentRunCallbacks): Promise<AgentRunResult> {
    ...
    try {
      const result = await this.client.run({
        ...
        onEvent: async (event) => {
          this.options.repositories.runs.addSdkEvent(run.id, session.id, event.type === "error" ? "error" : "info", event.type, event);
  ...
  private async handleClientEvent(event: PiStreamEvent, callbacks: AgentRunCallbacks, toolMessages: string[]): Promise<void> {
