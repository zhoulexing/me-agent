...
export class RunsRepository {
  ...
  addSdkEvent(runId: number, sessionId: number, level: string, eventName: string, payload: unknown): void {
    this.db
      .prepare(
        `INSERT INTO sdk_events (run_id, session_id, level, event_name, sdk_payload, created_at)
         VALUES (?, ?, ?, ?, ?, ?)`
      )
      .run(runId, sessionId, level, eventName, JSON.stringify(payload), nowIso());
  }
